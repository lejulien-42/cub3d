...
/*   Updated: 2020/02/28 21:41:21 by lejulien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	ft_set_texture(t_mlx_data *data)
{
	...
	if (data->what == '1')
		...
		color = data->s_fl.data[((int)data->texposs_fl &
		(data->s_fl.height - 1)) * data->s_fl.height + data->texxs_fl];
	else if (data->what == '7')
		color = data->s_arrowtex.data[((int)data->texposs_arrowtex &
		(data->s_arrowtex.height - 1)) * data->s_arrowtex.height +
		data->texxs_arrowtex];
	else if (data->what == '8')
		color = data->s_arrowtexl.data[((int)data->texposs_arrowtexl &
		(data->s_arrowtexl.height - 1)) * data->s_arrowtexl.height +
		data->texxs_arrowtexl];
	else if (data->what == '9')
		color = ft_do_ninetext(data);
	else
		color = ft_do_ninetexttwo(data);
	return (color);
}
